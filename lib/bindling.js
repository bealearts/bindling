...
// Polyfill until ES6 Proxy lands
...
function parse(watchers, element, attrName, textNode, value, model)
{
	...
	var watch = {
		...
		attrName: attrName,
		textNode: textNode,
		renderer: isOnlyOneParam(params, value) ? bindFactory(params(value)) : _template(value, {interpolate: bindingMatch})
	};
...
{
	return _uniq(matcher(value, {interpolate: bindingMatch}));
}
	...
	element.addEventListener(event, function onEvent(event) {
		...
		args.forEach(function(arg){
			if (arg)
			{
				...
				{
					argValues.push( convertSimpleType(arg) );
				}
			}
		});
...
function convertSimpleType(string)
{
	if (isNaN(+string))
	{
		return +string;
